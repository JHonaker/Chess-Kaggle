...
test_that("parse.game assigns winner", {
    expect_equal(parse.game('1-0')$winner, "White")
    expect_equal(parse.game('0-1')$winner, "Black")
})

test_that("parse.game handles a non-terminal game", {
    pos <- list(piece1='a2', piece2='b2')
    game_to <- function(moves) parse.game(game=pos, moves)

    moves <- c('a2a3', 'b2b3')

    expect_equal(game_to(moves), list(piece1='a3', piece2='b3'))
})
